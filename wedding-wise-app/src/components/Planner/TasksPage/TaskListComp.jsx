...
function TasksListComp() {
  ...
  const handleAddTask = async () => {
    try {
      const data = await sendData(
        ...
        "POST",
        {
          taskID: 0, // or whatever the initial value should be
          taskName: newTask,
  ...
  const checkedTasks = tasks ? tasks.filter((task) => task.completed).length : 0;
            ...
            <List>
              {currentSubTask.subTasks &&
                currentSubTask.subTasks.map((subtask, idx) => (
                  <ListItem key={`${currentSubTask.taskID}-${subtask.subTaskId}`}>
