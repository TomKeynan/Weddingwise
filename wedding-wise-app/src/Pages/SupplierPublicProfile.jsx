...
import { onSnapshot, doc, getDocs, where, collection, query } from "firebase/firestore";
...
function SupplierPublicProfile() {
  ...
  const { suppliers, loading, fetchSupplierData, setCurrentSupplierEmail } = useSupplierStore();

  // Retrieve email from sessionStorage
  const currentSupplierEmail = sessionStorage.getItem('currentSupplierEmail');
  console.log(currentSupplierEmail);

  // Set the email in Zustand store if it's not already set
  useEffect(() => {
    if (currentSupplierEmail) {
      fetchSupplierData(currentSupplierEmail);
    }
  }, [currentSupplierEmail, fetchSupplierData, setCurrentSupplierEmail]);
